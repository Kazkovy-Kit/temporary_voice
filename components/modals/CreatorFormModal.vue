...
const { data: categories } = await useAsyncData<Response>("categories", () =>
    $fetch(`/api/guilds/categories`),
);
...
const onSubmit = form.handleSubmit(async (values: any) => {
    await $fetch("/api/temporary-voice/creator/" + props.creator.id + "/edit", {
        method: "POST",
        body: values,
    });
    toast({
    ...
    <Dialog :open="creator !== null" @update:open="emits('update:open')">
        ...
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Додайте новий список правил для каналу з якого будуть
                    утворюватись тимчасові канали.
                </DialogDescription>
            </DialogHeader>
            <form id="creatorForm" @submit="onSubmit">
                ...
                <Field v-slot="{ componentField }" name="source_parent_id">
                    <FormItem>
                        <FormLabel> Основна категорія для каналу </FormLabel>
            ...
            <DialogFooter>
                <Button type="submit" form="creatorForm"> Зберегти </Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
</template>
